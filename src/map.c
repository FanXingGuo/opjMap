...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	if(n_a<-1) {
		...
		idxPos*windArrIdxPosP = (idxPos*)kmalloc(b->km,windLen * sizeof(idxPos));  // calloc会自动初始化为0
		...
		for (i = 0; i < n_a; ++i) {
			...
			tempX=ref_cur[a[i].x<<1>>33]+(int32_t)a[i].x;
			aw[i]=(uint32_t)(0.5*(tempX-(int32_t)a[i].y+10000000)/1000);//  max read length 1000w

			windArrIdxPosP[aw[i]].ref_id=a[i].x<<1>>33;
			if(windArrIdxPosP[aw[i]].st>(int32_t)a[i].x) {
				windArrIdxPosP[aw[i]].st=(int32_t)a[i].x;
			}
			if(windArrIdxPosP[aw[i]].ed<(int32_t)a[i].x) {
				windArrIdxPosP[aw[i]].ed=(int32_t)a[i].x;
			}

			// if(a[i].x>>63==0) { //strand +/-
			// 	windArrIdxPosP[aw[i]].freq+=1;
			// }
			// else {
			// 	windArrIdxPosP[aw[i]].freq+=1;
			// }
		...
		if(op9m) {
			...
			// printf("p%d-%d;",top1.ed-top1.st,top1.freq);
		...
		kfree(b->km,windArrIdxPosP);
		kfree(b->km,windBoolOpen);
		// kfree(b->km,windLenList);
		kfree(b->km,ref_cur);
