...
int mm_write_sam_hdr(const mm_idx_t *idx, const char *rg, const char *ver, int argc, char *argv[])
{
	...
	mm_sprintf_lite(&str, "@PG\tID:opjMap\tPN:opjMap");
...
void unsigned_int_to_str(unsigned int num, char *str,int qlen) {
	if (num == 0) {
		str[0] = '0';
		str[1] = '\0';
		return;
	...
	int i = 0;
	char temp_str[20]; // 临时缓冲区，用于存储反序的数字
	int temp_i = 0;
	if(num>=qlen)num-=qlen;

	// 将数字按位取模，得到反序的字符
	while (num > 0) {
		temp_str[temp_i++] = (num % 10) + '0';
		num /= 10;
	}

	// 将反序的字符重新排序并存入目标字符串
	while (temp_i > 0) {
		str[i++] = temp_str[--temp_i];
	}

	str[i] = '\0'; // 添加字符串结束符
}

static void write_sam_cigar(kstring_t *s, int sam_flag, int in_tag, int qlen, const mm_reg1_t *r, int64_t opt_flag,int ql, const mm_bseq1_t *t)
{
	if (r->p == 0) {
		mm_sprintf_lite(s, "*");
	} else {
		...
		if (in_tag) {
			...
			// if (clip_len[0]>=qlen)clip_len[0]-=qlen;
			assert(clip_len[0] < qlen && clip_len[1] < qlen);
			//printf("%d%c",clip_len[0],clip_char);
			// if (clip_len[0]&&((r->qe)*2 < qlen)) {
			// 	clip_len[0] = r->rev? (qlen>>1)-(r->qe) : r->qs;
			// 	if (clip_len[0]) mm_sprintf_lite(s, "%d%c",  clip_len[0], clip_char);
			// }else {
			// 	mm_sprintf_lite(s, "%d%c", clip_len[0], clip_char);
			// }
			// printf("%d\n",ql);
			// if (clip_len[0]) mm_sprintf_lite(s, "%d%c",  qlen, clip_char);
			// if (clip_len[0]) mm_sprintf_lite(s, "%d%c", r->qs, clip_char);
			// if (clip_len[0]) mm_sprintf_lite(s, "%d%c", r->qe, clip_char);
			//  if (clip_len[0]<=qlen) {
			...
			//
			//  	const int cpl=clip_len[0]+qlen;
			//  	char result_str[20];
			//  	unsigned_int_to_str(cpl, result_str,qlen);
			//  	if(qlen==6688)fprintf(stderr, "===========\n%d,%d,%s\n",clip_len[0],cpl,result_str);
			//
			//  	mm_sprintf_lite(s,result_str);
			//  	mm_sprintf_lite(s,"%c",clip_char);
			//
			...
			assert(clip_len[0] < qlen && clip_len[1] < qlen);
			if (clip_len[0]) mm_sprintf_lite(s, "%d%c", clip_len[0], clip_char);
			// if (clip_len[0]) mm_sprintf_lite(s, "%s",t->seq);
			for (k = 0; k < r->p->n_cigar; ++k)
				mm_sprintf_lite(s, "%d%c", r->p->cigar[k]>>4, MM_CIGAR_STR[r->p->cigar[k]&0xf]);
...
void mm_write_sam3(kstring_t *s, const mm_idx_t *mi, const mm_bseq1_t *t, int seg_idx, int reg_idx, int n_seg, const int *n_regss, const mm_reg1_t *const* regss, void *km, int64_t opt_flag, int rep_len)
{
	...
	if (r == 0) {
		...
		} else write_sam_cigar(s, flag, 0, t->l_seq, r, opt_flag,strlen(t->seq),t);
	}
	...
	if (r) {
		...
		if (cigar_in_tag)
			write_sam_cigar(s, flag, 1, t->l_seq, r, opt_flag,strlen(t->seq),t);
	}
	if (rep_len >= 0) mm_sprintf_lite(s, "\trl:i:%d", rep_len);
